...
{	
	...
	protected function _buildQuery() {
		...
		if (empty($from) && !empty($to)) {
			...
			$query .= ' AND '.$this->_db->qn('e.end').' <> '.$this->_db->q($this->_db->getNullDate()).' ';
		
			if (!empty($includeTo)) {
				$query .= ' AND ('.$this->_db->qn('e.end').' <= '.$this->_db->q($to).' OR '.$this->_db->qn('e.id').' IN ('.implode(',',$includeTo).')) ';
			} else {
				$query .= ' AND '.$this->_db->qn('e.end').' <= '.$this->_db->q($to).' ';
			}
			...
			if (!empty($includeFrom)) {
				$query .= ' AND ('.$this->_db->qn('e.start').' >= '.$this->_db->q($from).' OR '.$this->_db->qn('e.id').' IN ('.implode(',',$includeFrom).')) ';
			} else {
				$query .= ' AND '.$this->_db->qn('e.start').' >= '.$this->_db->q($from).' ';
			}
		} elseif (!empty($from) && !empty($to)) {
			...
			if (!empty($includeFromTo)) {
				...
				$query .= ' AND (('.$this->_db->qn('e.start').' <= '.$this->_db->q($from).' AND '.$this->_db->qn('e.end').' >= '.$this->_db->q($to).') OR ('.$this->_db->qn('e.start').' >= '.$this->_db->q($from).' AND '.$this->_db->qn('e.end').' <= '.$this->_db->q($to).')) ';
			}
		}
