	...
	<form id="rscommentsForm" name="rscommentsForm" action="javascript:void(0)" method="post">
		...
		<div class="row">
			<div class="col-lg-6 pb-3">
				...
				<?php } ?>
				<div class="controls">
					<input <?php echo $this->disable; ?> type="text" class="span11 required" required id="rsc_name" name="jform[name]" value="<?php echo $this->user->get('name'); ?>" size="45" <?php if (!$this->config->show_labels) { ?>placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_NAME').$required; ?>"<?php } ?> />
				</div>
			</div>
			<div class="col-lg-6  pb-3">
				<?php if ($this->config->show_labels) { ?>
				...
				<?php } ?>
				<div class="controls">
					<input <?php echo $this->disable; ?> type="text" class="span11 required" required id="rsc_email" name="jform[email]" value="<?php echo $this->user->get('email'); ?>" size="45" <?php if (!$this->config->show_labels) { ?>placeholder="<?php echo JText::_('COM_RSCOMMENTS_COMMENT_EMAIL').$required; ?>"<?php } ?> />
				</div>
			</div>
		</div>
		...
		<div class="row">
			...
			<?php } ?>
			<div class="col-lg-12 pb-3" id="rscomments-comment-area">
				<div class="controls">
					<textarea id="rsc_comment" required class="input-block-level required" name="jform[comment]" rows="5" onkeydown="rsc_comment_cnt('rsc_comment','commentlen','<?php echo $clength; ?>');" onkeyup="rsc_comment_cnt('rsc_comment','commentlen','<?php echo $clength; ?>');" maxlength="<?php echo $clength; ?>" placeholder="<?php echo JText::_('Напишите ваш коментарий'); ?>"></textarea>
				</div>
			</div>
		</div>
		...
		<div class="row-fluid">
			...
			<button type="button" id="rsc_submit" class="sppb-btn sppb-btn-md sppb-btn-primary sppb-btn-rounded sppb-btn-gradient" onclick="rsc_validate('<?php echo $upload; ?>', '<?php echo RSCommentsHelper::route('index.php?option=com_rscomments&task=captcha'); ?>');"><?php echo JText::_('COM_RSCOMMENTS_SEND'); ?></button>
			<button type="button" onclick="rsc_reset_form();" id="rsc_reset" class="sppb-btn sppb-btn-md sppb-btn-rounded sppb-btn-gradient"><?php echo JText::_('COM_RSCOMMENTS_RESET'); ?></button>
			<button type="button" id="rsc_cancel" class="sppb-btn sppb-btn-md sppb-btn-rounded sppb-btn-gradient" style="display:none;" onclick="rsc_cancel_reply();"><?php echo JText::_('COM_RSCOMMENTS_CANCEL'); ?></button>
		</div>
