...
class JSecurImage {
	...
	*/
	...
	function __construct()
	{
		...
		$this->wordlist_file = JPATH_SITE.'/components/com_rscomments/helpers/securimage/words/words.txt';
		$this->use_wordlist  = false;

		$this->gd_font_file  = JPATH_SITE.'/components/com_rscomments/helpers/securimage/gdfonts/automatic.gdf';
		$this->use_gd_font   = false;
		$this->gd_font_size  = 24;
		$this->text_x_start  = 15;

		$this->ttf_file      = JPATH_SITE.'/components/com_rscomments/helpers/securimage/AHGBold.ttf';
		...
		$this->signature_font  = JPATH_SITE.'/components/com_rscomments/helpers/securimage/AHGBold.ttf';

		$this->audio_path   = JPATH_SITE.'/components/com_rscomments/helpers/securimage/audio/';
	 ...
	 */
	function check($code, $type = 'default')
	{
	...
	function saveData()
	{
		...
		$type = JFactory::getApplication()->input->get('type','default');
		
		$session->set('com_rscomments.securimage.'.$type.'.value', $this->code);
		$session->set('com_rscomments.securimage.'.$type.'.ctime', time());
	}
	...
	function validate()
	{
		...
		$session = JFactory::getSession();
		$type = $this->captcha_type;
		
		$code = $this->getCode($type);
		if ($code != '' && $this->isCodeExpired($type))
			$code = '';
		
		$code_entered = $this->code_entered;
		if (!RSCommentsHelper::getConfig('captcha_cases'))
		{
		...
		if ($code != '' && $code == $code_entered)
		{
			...
			$session->set('com_rscomments.securimage.'.$type.'.value', '');
			$session->set('com_rscomments.securimage.'.$type.'.ctime', '');
		}
	}
	 ...
	 * @return string
	 */
	function getCode($type = 'default')
	{
		$session = JFactory::getSession();
		return $session->get('com_rscomments.securimage.'.$type.'.value', '');
	}
	 ...
	 */
	function isCodeExpired($type = 'default')
	{
		$session = JFactory::getSession();
		$creation_time = $session->get('com_rscomments.securimage.'.$type.'.ctime', 0);
		$expired = true;
