...
class RSFilterBar {
	...
	public $listOrder = '';
	// show the component filter
	public $ComponentFilter = null;
	// show the select article button
	public $ComponentArticleFilter = '';
	// show the No article button
	...
	public function show() {
		if ($this->sortFields || $this->orderDir) {
			JFactory::getDocument()->addScript(JURI::root(true).'/administrator/components/com_rscomments/assets/js/ordertable.js');
		}
		?>
		<div id="filter-bar" class="btn-toolbar">
			...
			<?php if (isset($this->Components)) { ?>
			<div class="btn-group pull-right hidden-phone">
					<select name="filter_component" class="inputbox" onchange="this.form.submit()">
						<?php echo JHtml::_('select.options', $this->Components, 'value', 'text', $this->ComponentFilter, false); ?>
					</select>
			</div>
			<?php } ?>
			
			<?php if (isset($this->ComponentFilter)) { ?>
			<div class="btn-group pull-right hidden-phone">	
				<?php echo $this->ComponentArticleFilter;?>
			</div>
			<?php } ?>
			
			<?php if (isset($this->selectNone)) { ?>
			<div class="btn-group pull-right">	
				<a class="rsc_filter_option btn" href="javascript:void(0)" rel=""><?php echo JText::_('COM_RSCOMMNETS_SELECT_NONE'); ?></a>
			</div>
			<?php } ?>
