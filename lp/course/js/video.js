...
let fs = 0;
let rabotaem;
let calcdlitelnost = (0*60*60*1000) + (49*60*1000)+(50*1000);

let nachalo = (18*(60*60*1000))+(25*(60*1000))+(0*(1000));
...
function showTime() {
    ...
    if ((vremyaNow() - nachalo >0) && (vremyaNow() - nachalo < calcdlitelnost) ) {
        ...
        rabotaem = false;
        p.innerHTML = ('<br>До начала вебинара: ' + (Math.floor(Math.abs(nachalo - vremyaNow())/(1000*60*60))) + "ч " + Math.floor((((nachalo - vremyaNow())/60)/1000)%60) + "м " + (((nachalo - vremyaNow())/1000)%60) + "с");
        // вырубаем контрол и автоплей
    }
    else{
        rabotaem = false;
        p.innerHTML = 'Вебинар окончен спасибо что пришли';}
    if((rabotaem==true) && (fs==0)){peremotka();vid.autoplay=true;fs=fs+1;}
    return rabotaem;
}
/**
 * @license
