...
class Data
{
	...
	public static function setDataOpen(&$data, $attributes = null)
	{
		...
		if (self::isOpen($values, $opentype))
		{
			$data['open'] = 'true';
		}
	}

	public static function isOpen($values, $opentype)
	{
		...
		{
			$count = self::getOpenCount($opentype);
		...
		{
			$open = self::getIsOpenFromValue($value, $opentype);
	...
	public static function getIsOpenFromValue($value, $opentype)
	{
	...
	private static function getOpenCount($type = '')
	{
		$params = Params::get();
		...
		$count++;
		$ttl = $params->open_count_ttl ? $params->open_count_ttl * 60 : (365 * 24 * 60 * 60); // default: 1 year
