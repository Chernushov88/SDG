...
(function($) {
	...
	RegularLabsModals = {
		...
		init: function(options) {
			...
			if (options.open_by_url) {
				$.each($('.' + options.class), function(i, el) {
					var $el = $(el);

					var href = $el.attr('href') !== 'undefined' ? $el.attr('href').replace(/.*?\#/g, '') : '';

					if (options.open_by_url == $el.attr('id')
						|| options.open_by_url == $el.attr('data-modal-rel')
						|| options.open_by_url == href.replace(/.*?\#/g, '')) {
						$el.click();
					}
				});
			}
		...
		initModal: function(el) {
			...
			defaults['onOpen'] = function() {
				clearTimeout(RegularLabsModals.modal_delay);

				if (options.disable_background_scrolling) {
			...
			if (!options.disable_on_mobile_splash || $(window).width() > options.mobile_max_width) {
				if (defaults['delay'] != undefined) {
					defaults['returnFocus'] = false;
				}
			}
			...
			if (!options.disable_on_mobile_splash || $(window).width() > options.mobile_max_width) {
				if (defaults['delay'] != undefined) {
					RegularLabsModals.modal_delay = setTimeout(function() {
						$el.click();
					}, defaults['delay']);
				}
			}
			...
			if (options.disable_on_mobile) {
				if ($(window).width() <= options.mobile_max_width) {
					$el.colorbox.remove();
